...
**Action**: Create a function to parse winning goalie information from schedule game data.

**Details**:
- Create `parse-winning-goalie` function similar to `parse-player-name` pattern
- Extract `player-id`, `first-initial`, and `last-name` from schedule game
- Format as: `{:player-id <id>, :player "<first-initial> <last-name>"}`
...
**Rationale**: Standardize the goalie data format to match the existing player name pattern used for goal scorers.
...
- The field follows the same localized string pattern as other player names
- Consider consistency with existing player name formatting (e.g., goal scorers use "First Last" format)
